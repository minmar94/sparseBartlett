...
Rcpp::List update_G_Rcpp(
    ...
    adjacency matrix.
    */
    if (adj.nrow() == 1) return Rcpp::List::create(adj);  // No update if there is only one node
    sfc64 rng(seed);
    ...
    return Rcpp::List::create(Blaze2R(adj_Blaze), Blaze2R(K));
...
//Rcpp::List update_G_Rcpp_MingGianPier(
//    Rcpp::NumericMatrix adj, Rcpp::NumericMatrix edge_prob_mat, double df,
//    double df_0, Rcpp::NumericMatrix rate, int n_edge, int seed,
//    bool approx = false, bool delayed_accept = true, bool loc_bal = true
//) {
...
//    return Rcpp::List::create(adj_Blaze, K);
//}
