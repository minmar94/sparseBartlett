...
k <- 50
load(paste0("WSReal/GeneExp", k, "_Confronti_GW.Rdata")) # cambiare l'RData
...
# Poisson: doubs ----------------------------------------------------------
rm(list = ls()[!(ls() %in% c("draw_lambda", "draw_zeta"))])
load("WSReal/doubs_indipendent_speciesmcmc_out_lambda.Rdata") # cambiare l'RData

doubs <- readxl::read_excel("doubs_data.xlsx") %>% filter(Site != 8)
